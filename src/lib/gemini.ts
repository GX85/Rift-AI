...
function polishedLocalFallback(prompt: string, _system: string): string {
  ...
  const isSite = /(сайт|лендинг|landing|html|верстк|website|страниц|web-app|проект|продукт|прототип|mvp|прилож|программ|утилит|сервис|панел|dashboard|трекер|todo)/i.test(text);
...
<head>
  ...
  <style>
    *{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;place-items:center;background:radial-gradient(circle at 50% 0,#ffffff14,transparent 34%),repeating-linear-gradient(0deg,#ffffff0d 0 1px,transparent 1px 36px),repeating-linear-gradient(90deg,#ffffff0d 0 1px,transparent 1px 36px),#050506;color:white;font-family:Inter,Arial,sans-serif;overflow:hidden}.wrap{width:min(96vw,980px)}canvas{width:100%;aspect-ratio:16/9;border:1px solid #ffffff26;border-radius:22px;background:#09090b;box-shadow:0 30px 90px #0009;touch-action:none}.hud{display:flex;justify-content:space-between;gap:12px;margin:12px 4px;color:#d4d4d8}.pad{display:none;grid-template-columns:repeat(3,56px);gap:10px;justify-content:center;margin-top:12px}.pad button{height:52px;border:1px solid #ffffff2b;border-radius:16px;background:#101012;color:white;font-size:20px}@media(max-width:720px){.pad{display:grid}.hud{font-size:14px}}
  </style>
</head>
<body>
  ...
  <script>
    ...
    function draw(){x.clearRect(0,0,960,540);let g=x.createLinearGradient(0,0,960,540);g.addColorStop(0,'#111113');g.addColorStop(1,'#050506');x.fillStyle=g;x.fillRect(0,0,960,540);for(let i=0;i<70;i++){x.fillStyle='#ffffff12';x.fillRect((i*97+t*.02)%960,(i*53)%540,2,2)}orbs.forEach(o=>{x.fillStyle='hsl(0,0%,'+(58+Math.sin(o.h)*18)+'%)';x.beginPath();x.arc(o.x,o.y,o.r,0,7);x.fill()});shots.forEach(s=>{x.fillStyle='#f4f4f5';x.fillRect(s.x-3,s.y-18,6,22)});parts.forEach(p=>{x.globalAlpha=p.a;x.fillStyle=p.c;x.fillRect(p.x,p.y,4,4);x.globalAlpha=1});if(player){x.fillStyle='#f4f4f5';x.beginPath();x.moveTo(player.x,player.y-24);x.lineTo(player.x-22,player.y+22);x.lineTo(player.x+22,player.y+22);x.closePath();x.fill()}x.fillStyle='white';x.font='22px Arial';x.fillText('Score '+score,24,34);x.fillText('Lives '+lives,24,64);if(state!=='play'){x.textAlign='center';x.font='54px Arial';x.fillText(state==='over'?'Game Over':'Amethyst Runner',480,240);x.font='22px Arial';x.fillText('Space/тап — старт и огонь, R — рестарт',480,282);x.textAlign='left'}}
...
<style>
body{margin:0;min-height:100vh;display:grid;place-items:center;background:#050506;color:#fff;font-family:Inter,Arial}.chat{width:min(460px,94vw);height:min(720px,92vh);display:grid;grid-template-rows:auto 1fr auto;border:1px solid #ffffff1f;border-radius:24px;background:#111113;overflow:hidden}.top{padding:16px 18px;border-bottom:1px solid #ffffff12;font-weight:900}.log{padding:16px;overflow:auto;display:flex;flex-direction:column;gap:10px}.msg{max-width:84%;padding:10px 12px;border-radius:16px;background:#202024}.bot{align-self:flex-start}.user{align-self:flex-end;background:#f5f5f5;color:#111}.bar{display:flex;gap:8px;padding:12px;border-top:1px solid #ffffff12}input{flex:1;border:0;border-radius:14px;padding:12px;background:#ffffff10;color:#fff}button{border:1px solid #ffffff24;border-radius:14px;padding:0 14px;font-weight:800}
</style>
...
<head>
  ...
  <style>
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;font-family:Inter,Arial,sans-serif;background:#050506;color:#fff}nav{position:sticky;top:0;z-index:2;display:flex;justify-content:space-between;align-items:center;padding:14px clamp(16px,4vw,42px);background:#050506cc;backdrop-filter:blur(18px);border-bottom:1px solid #ffffff12}a{color:inherit}.brand{font-weight:950}.btn{display:inline-flex;align-items:center;justify-content:center;min-height:44px;padding:0 18px;border-radius:14px;text-decoration:none;font-weight:850}.primary{background:#fff;color:#111}.ghost{border:1px solid #ffffff24}.hero{min-height:82vh;display:grid;align-items:center;padding:clamp(34px,7vw,90px) clamp(16px,5vw,72px);background:radial-gradient(circle at 72% 24%,#ffffff12,transparent 28%),repeating-linear-gradient(0deg,#ffffff0b 0 1px,transparent 1px 40px),repeating-linear-gradient(90deg,#ffffff0b 0 1px,transparent 1px 40px),#050506}section{padding:64px clamp(16px,5vw,72px)}.wrap{width:min(1120px,100%);margin:auto}.eyebrow{color:#f5f5f5;font-weight:900}h1{max-width:900px;font-size:clamp(42px,9vw,104px);line-height:.94;margin:12px 0;letter-spacing:0}p{max-width:680px;color:#d4d4d8;font-size:clamp(16px,2vw,20px);line-height:1.6}.cta{display:flex;gap:12px;flex-wrap:wrap;margin-top:26px}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px}.card{padding:22px;border:1px solid #ffffff1f;border-radius:18px;background:#101012;box-shadow:0 20px 60px #0006}.card b{display:block;margin-bottom:8px}.footer{border-top:1px solid #ffffff12;color:#a1a1aa}@media(max-width:640px){nav{padding:10px 14px}.hero{min-height:auto;padding-top:54px}.cta .btn{width:100%}section{padding-block:42px}}
  </style>
</head>
<body>
...
function svgImageFallback(text: string): string {
  ...
  <defs>
    <linearGradient id="bg" x1="0" y1="0" x2="1" y2="1">
      ...
      <stop offset="0.55" stop-color="#171719"/>
  ...
  <path d="M512 126 716 410 590 884 316 602z" fill="#f4f4f5" opacity=".24" filter="url(#glow)"/>
  <path d="M512 126 590 884 792 522z" fill="#d4d4d8" opacity=".22"/>
  <path d="M316 602 512 126 232 480z" fill="#71717a" opacity=".18"/>
  ...
  <foreignObject x="172" y="820" width="680" height="120">
    <div xmlns="http://www.w3.org/1999/xhtml" style="color:#f4f4f5;font:24px Arial;text-align:center;line-height:1.3">${safeText}</div>
  </foreignObject>
</svg>`;
