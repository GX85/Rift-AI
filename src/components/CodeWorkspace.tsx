...
- Не вставляй code block, HTML, JSON, CSS или JS без явной просьбы о коде.
- Не превращай вопрос в шаблон сайта/игры. Отвечай по сути вопроса обычным русским текстом.`;
  }
...
function localConversationAnswer(text: string) {
  ...
  if (/^(как дела|как ты|ты тут|давай поговорим|можешь поговорить)[?.!\s]*$/.test(normalized)) {
    return 'Я тут. Могу просто поговорить без кода, помочь сформулировать идею, подготовить презентацию или перейти в режим разработки, когда ты прямо попросишь создать сайт, игру или приложение.';
  }
  if (/^(кто ты|что такое amethyst|что такое аметист)[?.!\s]*$/.test(normalized)) {
    return 'Я Amethyst: ИИ-ассистент для идей, объяснений, кода, сайтов, игр и MVP. Если вопрос обычный, отвечаю обычным текстом. Если ты просишь создать продукт, собираю рабочий результат.';
  }
  if (/(почему|зачем|снова|опять).*(код|html)|всучиваешь|пишешь код|не просил код/.test(normalized)) {
    return 'Да, это было неправильное поведение. Теперь я отвечаю без кода на обычные вопросы и включаю генерацию только по явной просьбе: создать сайт, игру, приложение, компонент, исправить код или провести ревью.';
  }
...
<head>
  ...
  <title>${title}</title>
  <style>
    *{box-sizing:border-box}body{margin:0;font-family:Inter,Arial,sans-serif;background:#050506;color:#fff}button,input,textarea{font:inherit}.shell{min-height:100vh;padding:24px;background:radial-gradient(circle at 72% 18%,#ffffff12,transparent 28%),repeating-linear-gradient(0deg,#ffffff0b 0 1px,transparent 1px 40px),repeating-linear-gradient(90deg,#ffffff0b 0 1px,transparent 1px 40px),#050506}.wrap{width:min(1120px,100%);margin:auto}.nav{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:56px}.brand{font-weight:950}.btn{display:inline-flex;min-height:44px;align-items:center;justify-content:center;border:1px solid #ffffff24;border-radius:14px;padding:0 18px;background:#f5f5f5;color:#111;text-decoration:none;font-weight:850}.hero{display:grid;grid-template-columns:1.1fr .9fr;gap:24px;align-items:center}.panel,.card{border:1px solid #ffffff1f;border-radius:22px;background:#101012;box-shadow:0 24px 80px #0007}.panel{padding:22px}.eyebrow{color:#f5f5f5;font-weight:900}h1{font-size:clamp(42px,8vw,88px);line-height:.94;margin:12px 0}p{color:#d4d4d8;line-height:1.6}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:12px;margin-top:26px}.card{padding:18px}.app{display:grid;gap:12px}.row{display:flex;gap:8px}.row input{flex:1;min-width:0;border:1px solid #ffffff1f;border-radius:14px;background:#08080a;color:#fff;padding:12px}.list{display:grid;gap:8px}.item{display:flex;justify-content:space-between;gap:8px;padding:12px;border-radius:14px;background:#ffffff10}@media(max-width:760px){.shell{padding:14px}.nav{margin-bottom:32px}.hero{grid-template-columns:1fr}.btn{width:100%}.row{flex-direction:column}}
  </style>
</head>
<body>
  <main class="shell">
...
• Первый экран не должен быть пустым: всегда добавляй видимый hero/меню/сцену, рабочие кнопки, обработчики событий и mobile layout.
• Если создаешь игру, обязательно добавь старт, паузу/рестарт, счет, collision logic, keyboard + touch управление и requestAnimationFrame.
...
Формат:
• CHAT: 2-6 коротких предложений обычным русским текстом, без кода.
• ARTIFACT: короткий результат, затем рабочий код в markdown-блоке, затем как открыть/проверить.
• REVIEW/FIX: причина, фикс, проверка.`;
}
